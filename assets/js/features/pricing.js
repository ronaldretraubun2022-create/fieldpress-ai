...
import { PRICING_PLANS, TOPUP_PACKAGES, whatsappLink } from "./features/pricing-config.js";
...
function planCard(plan) {
  return `
    <article class="rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-2xl">
      ...
      <p class="mt-2 min-h-[48px] text-slate-400">${plan.description}</p>
      <p class="mt-6 text-4xl font-black text-cyan-300">${plan.price}</p>
...
function topupCard(pkg) {
  return `
    <article class="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
      ...
      <p class="mt-3 text-3xl font-black text-cyan-300">${pkg.price}</p>
...
function bindButtons() {
  document.querySelectorAll(".buy-plan-btn").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      const message = `Saya tertarik dengan FieldPress AI paket ${plan.name} (${plan.price}). Mohon info cara aktivasi.`;

      await createBillingRequest(
        "plan",
        { planCode: plan.code, amountIdr: 0 },
        message
      );
    });
  });
  ...
  document.querySelectorAll(".buy-topup-btn").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      await createBillingRequest(
        "topup",
        { topupCode: pkg.code, amountIdr: pkg.priceIdr },
        message
      );
    });
  });
