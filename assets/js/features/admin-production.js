...
function userRow(user) {
  ...
  return `
    <tr class="border-b border-white/5 align-top">
      ...
      <td class="py-4 pr-4"><select data-user-field="role" data-user-id="${user.id}" class="user-admin-input w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm outline-none focus:border-cyan-400">${optionList(ROLES, user.role || "user")}</select></td>
      <td class="py-4 pr-4"><select data-user-field="plan" data-user-id="${user.id}" class="user-admin-input w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm outline-none focus:border-cyan-400">${optionList(PLANS, user.plan || "free")}</select></td>
      <td class="py-4 pr-4"><select data-user-field="status" data-user-id="${user.id}" class="user-admin-input w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm outline-none focus:border-cyan-400">${optionList(STATUSES, status)}</select></td>
      <td class="py-4 pr-4"><input data-user-field="quota_override_ai" data-user-id="${user.id}" class="user-admin-input w-28 rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm outline-none focus:border-cyan-400" type="number" min="0" placeholder="-" value="${user.quota_override_ai ?? ""}" /></td>
      <td class="py-4 pr-4"><input data-user-field="quota_override_audio_minutes" data-user-id="${user.id}" class="user-admin-input w-28 rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm outline-none focus:border-cyan-400" type="number" min="0" placeholder="-" value="${user.quota_override_audio_minutes ?? ""}" /></td>
      ...
      <table class="w-full min-w-[1080px] text-left text-sm">
        <thead class="text-slate-400">
          <tr class="border-b border-white/10">
            <th class="py-3 pr-4">User</th><th class="py-3 pr-4">Role</th><th class="py-3 pr-4">Plan</th><th class="py-3 pr-4">Status</th><th class="py-3 pr-4">AI Override</th><th class="py-3 pr-4">Audio Override</th><th class="py-3 text-right">Action</th>
          </tr>
        </thead>
        <tbody>${filtered.length ? filtered.map(userRow).join("") : `<tr><td colspan="7" class="py-8 text-center text-slate-400">User tidak ditemukan.</td></tr>`}</tbody>
      </table>
    </div>
  `;
  ...
  target.innerHTML = `
    <section class="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
      <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div><h2 class="text-2xl font-black">User Management Admin</h2><p class="mt-2 text-sm text-slate-400">Cari user, ubah plan, ubah role, suspend user, dan reset quota.</p></div>
      ...
      <div id="userManagementTable" class="mt-6"><div class="rounded-2xl border border-white/10 bg-slate-950/70 p-5 text-slate-400">Loading users...</div></div>
    </section>
  `;
  try {
    state.users = await fetchUsers();
...
function renderChartBars(entries) {
  ...
  return `<div class="mt-5 flex h-64 items-end gap-3 rounded-3xl border border-white/10 bg-slate-950/60 p-5">${entries
    .map(([label, value]) => {
...
export async function exportRevenuePDF() {
  ...
  const html = `<html><head><title>FieldPress AI Revenue Report</title><style>body{font-family:Arial,sans-serif;padding:32px;color:#111827}table{width:100%;border-collapse:collapse;margin-top:24px}th,td{border:1px solid #e5e7eb;padding:8px;font-size:12px;text-align:left}th{background:#f3f4f6}.total{margin-top:16px;font-size:18px;font-weight:700}</style></head><body><h1>FieldPress AI Revenue Report</h1><p>Generated: ${new Date().toLocaleString("id-ID")}</p><p class="total">Total Approved Revenue: ${rupiah(total)}</p><table><thead><tr><th>Date</th><th>Type</th><th>Status</th><th>Plan</th><th>Top Up</th><th>Amount</th></tr></thead><tbody>${rows.map((row) => `<tr><td>${new Date(row.created_at).toLocaleString("id-ID")}</td><td>${row.request_type || ""}</td><td>${row.status || ""}</td><td>${row.plan_code || ""}</td><td>${row.topup_code || ""}</td><td>${rupiah(row.amount_idr)}</td></tr>`).join("")}</tbody></table></body></html>`;
